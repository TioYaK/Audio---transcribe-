document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            lines.forEach(line => {
                ...
                if (match) {
                    ...
                    // Make timestamp clickable link
                    const tsLink = `<span class="ts-link" onclick="seekTo(${sec})" title="Pular para ${match[0]}">${match[0]}</span>`;
                    displayLine = line.replace(match[0], tsLink);
                }
                // Mark lines with data-time even if 0
                htmlContent += `<p class="transcript-line" data-time="${sec}">${displayLine}</p>`;
            });
            ...
            if (notesArea) {
                ...
                // Debounced auto-save
                notesArea.oninput = debounce(async (e) => {
                    const val = e.target.value;
                    try {
                        const loadingIcon = document.querySelector('button[onclick*="notes"]'); // Hacky indicator
                        if (loadingIcon) loadingIcon.textContent = 'Anotações 📝 ...';

                        await authFetch(`/api/task/${id}/notes`, {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ notes: val })
                        });
                        if (loadingIcon) loadingIcon.textContent = 'Anotações 📝 (Salvo)';
                        setTimeout(() => { if (loadingIcon) loadingIcon.textContent = 'Anotações 📝'; }, 2000);
        ...
        setTimeout(() => {
            try {
                ...
                if (container.clientWidth === 0) {
                    console.warn("WaveSurfer container has 0 width. Modal might be hidden.");
                }
    ...
    // Global seek function for timestamp links
